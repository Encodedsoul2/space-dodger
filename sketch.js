...
const SAFE_MAX_ENEMIES=10;
const SAFE_MAX_SHOTS=60;
const SAFE_MAX_BULLETS=60;
const SAFE_MAX_PARTICLES=320;
const SAFE_MAX_FLOATING=24;
...
let objectiveTarget=500;
let objectiveProgress=0;
let objectiveComplete=false;

let enemiesDestroyedThisLevel=0;
let powerupsCollected=0;
let elitesDestroyedThisLevel=0;
let damageTakenThisLevel=0;

let waveNumber=0;
let waveActive=false;
let formationPulse=0;
let lastWaveAt=0;
let nextWaveDelay=1300;
...
let activeWeaponUntil=0;
let weaponLastFire=0;
...
function draw(){
  ...
  // Never replace the gameplay with a recovery/black screen.
  // If one non-critical subsystem throws, isolate that subsystem
  ...
  try{
    ...
    // Deliberately silent:
    // no GAME RECOVERING overlay and no black patch.
    // The next frame continues normally.
    resetMatrix();
  }

  hardResetTransform();
  lastFrameSeen=millis();
}
...
function loadSave(){
  try{
    ...
    unlockedLevel=sdClampInt(
      d.unlockedLevel||1,
      1,
      TOTAL_LEVELS
    );

    selectedShip=sdClampInt(
      d.selectedShip||0,
      0,
      SHIPS.length-1
    );

    if(unlockedLevel<SHIPS[selectedShip].unlock)
      selectedShip=0;

    campaignLives=sdClampInt(
      d.campaignLives||START_LIVES,
      START_LIVES,
      MAX_LIVES
    );
    ...
    swappedControls=d.swappedControls===true;
    musicOn=d.musicOn!==false;

    highestDragon=sdClampInt(
      d.highestDragon||0,
      0,
      TOTAL_LEVELS
    );

    dragonBadge=d.dragonBadge||"";
    dragonTitle=d.dragonTitle||"";
...
function startLevel(n){
  ...
  lastDropSpawn=millis();
  lastWeaponSpawn=millis();

  eventType="";
  eventUntil=0;

  nextEventAt=
    millis()+15000+random(9000);
  ...
  guaranteedSlayerSpawned=false;

  enemiesDestroyedThisLevel=0;
  powerupsCollected=0;
  elitesDestroyedThisLevel=0;
...
function playerSpeed(){
  ...
  if(shipPower()==="QUANTUM DASH")
    s*=1.3;

  if(shipPower()==="TITAN CORE")
    s*=.9;
...
function playerDamageMultiplier(){
  ...
  if(shipPower()==="BURN SHOT")
    d*=1.12;

  if(
    shipPower()==="DRAGON RAGE" &&
    bossActive
  )
    d*=1.75;
...
class SpacePlayer{
  ...
  update(){
    ...
    // INFINITE SPACE:
    // no invisible wall.
    // Leaving one side brings the ship back
...
function drawShield(x,y){
  ...
  circle(
    x,
    y,
    66+sin(frameCount*.08)*5
  );
}

function drawCryo(x,y){
  ...
  circle(
    x,
    y,
    98+sin(frameCount*.07)*8
  );
}

function runGame(){

  // Keep simulation/world rendering isolated
  // from the fixed HUD layer.

  if(enemies.length>SAFE_MAX_ENEMIES)
    enemies.length=SAFE_MAX_ENEMIES;

  if(enemyShots.length>SAFE_MAX_SHOTS)
    enemyShots.splice(
      0,
      enemyShots.length-SAFE_MAX_SHOTS
    );

  if(bullets.length>SAFE_MAX_BULLETS)
    bullets.splice(
      0,
      bullets.length-SAFE_MAX_BULLETS
    );

  if(particles.length>SAFE_MAX_PARTICLES)
    particles.splice(
      0,
      particles.length-SAFE_MAX_PARTICLES
    );

  if(floatingTexts.length>SAFE_MAX_FLOATING)
    floatingTexts.splice(
      0,
      floatingTexts.length-SAFE_MAX_FLOATING
    );
  ...
  S(updateKeyboardMovement);
  S(updateJoystickMovement);
  ...
  S(collideBulletsEnemies);
  S(collideShipEnemies);
  S(collideShipShots);
  S(collideBulletsBoss);

  S(updateObjective);
  S(checkLevelComplete);
  S(updateLiveEvent);

  // Draw player/world layer first.
  // Then explicitly restore screen-space coordinates.
  ...
  S(drawWeaponAttachment);

  // Static UI pass:
  // HUD, warnings and controls can NEVER
  // inherit any world/object rotation.
...
function updateWaves(){
  ...
  // FIX:
  // release the wave lock after a formation is cleared.
  ...
  if(
    enemies.length===0 &&
    !waveActive &&
    millis()-lastWaveAt>nextWaveDelay
  )
    startWave();
}
...
function startWave(){
  ...
  waveNumber++;
  waveActive=true;
  formationPulse=millis();

  const count=Math.min(
    14,
    3+
    Math.floor(level*.48)+
    Math.floor(waveNumber/3)+
    floor(random(2))
  );
...
function spawnFormation(count){
  ...
  // Guarantee at least one ELITE per wave
  // while an ELITE objective is unfinished.

  if(
    objectiveType==="ELITE" &&
    elitesDestroyedThisLevel<objectiveTarget &&
    !enemies.some(
      e=>e.type==="ELITE"
    )
  ){
...
function spawnWeaponDrops(){
  ...
  if(isDragonLevel(level)){
    ...
    if(
      ...
      weaponDrops.push(
        new WeaponDrop("SLAYER")
      );

      guaranteedSlayerSpawned=true;
...
class Enemy{
  ...
  constructor(x,y){
    ...
    // Multiple alien silhouettes can appear
    // in the same level and wave.
  ...
  update(){
    ...
    if(shipPower()==="GRAVITY PULSE")
      vy*=.55;

    if(shipPower()==="TIME FREEZE")
      vy*=.38;
...
function chooseEnemyType(){
  ...
  // FIX:
  // Level 4 (and later ELITE missions)
  // now actually spawn ELITEs.
...
function drawAlienVariant(v,type){
  ...
  const palettes=[
    ...
    ["#65dff5","#183f58","#c8fbff"],
    ["#ef5dce","#42143f","#ffe0f8"],
    ["#ffad43","#572b14","#fff0c7"],
    ["#ff5757","#4b1518","#ffb4b4"],
    ["#b879ff","#32184d","#f3d9ff"],
    ["#57ff9a","#103f2b","#d9ffea"],
    ["#62a9ff","#102d55","#dff1ff"],
    ["#ffdd52","#4d3b0b","#fff7c2"],
    ["#ff704d","#4d1d12","#ffe0d4"],
    ["#00e0c5","#073f3b","#e0fffa"],
    ["#9d7cff","#28164c","#efe5ff"],
    ["#ff5aa8","#4c1232","#ffe1f0"],
    ["#7cf7ff","#123b46","#efffff"],
    ["#f58cff","#40134b","#ffe9ff"],
    ["#8cff57","#183f12","#efffe4"],
    ["#ff9b55","#4a2510","#fff0df"],
    ["#5d8dff","#142550","#e4ecff"],
    ["#e8e05b","#3e3c0b","#ffffdf"],
    ["#ff5f7a","#46111d","#ffe2e7"],
    ["#d96cff","#36104b","#f5ddff"]
  ];
...
function checkLevelComplete(){
  ...
  // Dragon stages are boss stages:
  // only the Dragon kill can finish them.
  ...
  // Survival stages have a real timer.
...
function drawIntro(){
  ...
  // Intro intentionally stays until
  // player explicitly starts.
}
...
function drawAchievements(){
  ...
  // Backward-compatible achievement recovery.

  let inferred=0;

  for(
    const n of DRAGON_LEVELS
  )
    if(
      unlockedLevel>n
    )
...
function startMusic(){
  ...
  // Original liminal/arcade ambience.
